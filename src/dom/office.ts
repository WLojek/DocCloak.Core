 ...
 *
 * Ties the format modules (docx.ts, xlsx.ts, and the PDF module behind
 * '@doccloak/core/pdf', T215) to the detection pipeline and an
 ...
 *
 * The PDF module is imported lazily so hosts that never see a PDF do not
 * load pdf-lib, pdf.js and fontkit.
 */
...
import type { PdfReadOptions, PdfWriteOptions, RemovedPart } from '../pdf/types.ts';
import type { PdfExtraction } from '../pdf/read.ts';

/** File kinds the office redaction flow can regenerate. */
export type OfficeFileKind = 'docx' | 'xlsx' | 'pdf';
...
/** Lazy loader of the PDF module (keeps pdf-lib/pdf.js/fontkit out of hosts that never see a PDF). */
async function pdfModule(): Promise<typeof import('../pdf/index.ts')> {
  return import('../pdf/index.ts');
}

type AnyExtraction = DocxExtraction | XlsxExtraction | PdfExtraction;

async function readByKind(kind: OfficeFileKind, file: File, pdf?: PdfReadOptions): Promise<AnyExtraction> {
  if (kind === 'docx') return readDocx(file);
  if (kind === 'xlsx') return readXlsx(file);
  // A host that has not wired the PDF assets (pdf.js worker, fallback fonts) cannot redact PDFs; refuse
  // the way an unsupported format is refused instead of failing deep inside pdf.js.
  if (!pdf) throw new Error(`Unsupported office file: ${file.name} (this host has no PDF support configured)`);
  const { readPdf } = await pdfModule();
  return readPdf(file, pdf);
}
...
export interface OfficeAnalysis {
  ...
  warnings: string[];
  /** PDF only: parts the writer will drop (annotations, forms, bookmarks, ...). */
  removed?: RemovedPart[];
}

export interface AnalyzeOfficeFileOptions {
  /** PDF reader options (asset URLs for pdf.js and the fallback fonts, password, caps). */
  pdf?: PdfReadOptions;
}
 ...
 */
export async function analyzeOfficeFile(file: File, detect: DetectFn, options: AnalyzeOfficeFileOptions = {}): Promise<OfficeAnalysis> {
  const kind = officeFileKind(file.name);
  if (!kind) throw new Error(`Unsupported office file: ${file.name}`);
  const extraction = await readByKind(kind, file, options.pdf);
  ...
  return {
    ...
    removed: kind === 'pdf' ? (extraction as PdfExtraction).removed : undefined,
  };
}
...
export interface RedactOfficeFileOptions {
  ...
  /** PDF only: reader options (assets, password) and writer options (fit policy, raster DPI). */
  pdf?: PdfReadOptions & PdfWriteOptions;
}
...
export interface OfficeRedactionResult {
  ...
  unredactable: UnredactablePart[];
  /** PDF only: parts dropped from the output. */
  removed?: RemovedPart[];
  /** PDF only: 0-based pages that were rasterized because they could not be edited safely. */
...
export async function redactOfficeFile(
  ...
  let written: { blob: Blob; warnings: string[]; removed?: RemovedPart[]; rasterizedPages?: number[] };
  if (kind === 'docx') {
    written = await writeAnonymizedDocxWithReport(extraction as DocxExtraction, replacements, valueReplacements, {
      acceptTrackedChanges: options.acceptTrackedChanges ?? true,
      allowUnredactable: options.allowUnredactable ?? false,
    });
  } else if (kind === 'xlsx') {
    written = await writeAnonymizedXlsxWithReport(extraction as XlsxExtraction, replacements, valueReplacements, {
      allowUnredactable: options.allowUnredactable ?? false,
    });
  } else {
    const { writeAnonymizedPdfWithReport } = await pdfModule();
    written = await writeAnonymizedPdfWithReport(extraction as PdfExtraction, replacements, valueReplacements, {
      ...(options.pdf ?? {}),
      allowUnredactable: options.allowUnredactable ?? false,
    });
  }
  ...
  return {
    ...
    rasterizedPages: written.rasterizedPages,
  };
}
